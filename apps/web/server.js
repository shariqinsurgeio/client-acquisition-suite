...
// =============================================================================
const PlatformEnum = zod_1.z.enum(["UPWORK", "LINKEDIN", "FIVERR", "FREELANCER", "UNKNOWN"]);
const ExtensionConnectSchema = zod_1.z.object({
    extensionId: zod_1.z.string().min(1),
});
const DataIngestSchema = zod_1.z.object({
    platform: PlatformEnum.default("UNKNOWN"),
    title: zod_1.z.string().min(1).max(500).default("No Title"),
    description: zod_1.z.string().max(10000).default(""),
    ...
    clientLocation: zod_1.z.string().max(200).optional().nullable(),
    clientCountry: zod_1.z.string().max(100).optional().nullable(),
    clientTotalSpent: zod_1.z.number().optional().nullable(),
    clientAvgHourly: zod_1.z.number().optional().nullable(),
    clientHireRate: zod_1.z.number().optional().nullable(),
    clientPaymentVerified: zod_1.z.boolean().optional(),
    clientReviewCount: zod_1.z.number().optional().nullable(),
    // Job Metadata
    connectsCost: zod_1.z.number().optional().nullable(),
    postedAgo: zod_1.z.string().max(100).optional().nullable(),
    hasExternalLinks: zod_1.z.boolean().optional(),
    skillsRequired: zod_1.z.array(zod_1.z.string()).optional().nullable(),
    jobType: zod_1.z.string().max(50).optional().nullable(),
    experienceLevel: zod_1.z.string().max(100).optional().nullable(),
    projectLength: zod_1.z.string().max(100).optional().nullable(),
    // Refresh flag - indicates this is a refresh of existing job data
    isRefresh: zod_1.z.boolean().optional(),
});
const CmdExecuteSchema = zod_1.z.object({
    action: zod_1.z.enum(["SCRAPE", "SCRAPE_ALL", "APPLY", "REFRESH_JOBS", "ENRICH_JOBS"]),
    platform: PlatformEnum,
    targetUrl: zod_1.z.string().url().optional(),
    jobUrls: zod_1.z.array(zod_1.z.string().url()).optional(), // For REFRESH_JOBS and ENRICH_JOBS
    data: zod_1.z.record(zod_1.z.string(), zod_1.z.unknown()).optional(),
    // Multi-source scraping options
    scrapeMode: zod_1.z.enum(["all", "best-matches", "most-recent", "search"]).optional(),
    searchKeywords: zod_1.z.array(zod_1.z.string()).optional(), // For SCRAPE_ALL - array of keywords
});
// Discovery Complete - jobs found from multi-source scraping
const DiscoveryCompleteSchema = zod_1.z.object({
    jobs: zod_1.z.array(zod_1.z.object({
        url: zod_1.z.string().url(),
        title: zod_1.z.string().max(500).default("No Title"),
        description: zod_1.z.string().max(10000).default(""),
    ...
    sourceCount: zod_1.z.number(),
});
// Enrichment Complete - jobs enriched with full descriptions
const EnrichmentCompleteSchema = zod_1.z.object({
    enrichedJobs: zod_1.z.array(zod_1.z.object({
        url: zod_1.z.string().url(),
        fullDescription: zod_1.z.string().max(20000),
...
const TaskUpdateSchema = zod_1.z.object({
    ...
    jobUrl: zod_1.z.string().optional(), // For JOB_EXPIRED status
    status: zod_1.z.enum(["COMPLETED", "ERROR", "IN_PROGRESS", "JOB_EXPIRED"]),
    error: zod_1.z.string().optional(),
    scraped: zod_1.z.number().optional(),
    expired: zod_1.z.number().optional(), // Count of expired jobs
    failed: zod_1.z.number().optional(), // Count of failed jobs
    message: zod_1.z.string().optional(),
});
const ScrapeProgressSchema = zod_1.z.object({
    current: zod_1.z.number(),
    status: zod_1.z.string(),
});
// Anti-detection - SCRAPE_BLOCKED event from extension
const ScrapeBlockedSchema = zod_1.z.object({
    signal: zod_1.z.string(), // Detection signal type (cloudflareChallenge, captchaPresent, etc.)
    timestamp: zod_1.z.string(), // ISO timestamp
    action: zod_1.z.enum(["STOPPED", "COOLDOWN_STARTED"]),
    cooldownMinutes: zod_1.z.number().optional(),
    dailyCounters: zod_1.z.object({
        date: zod_1.z.string(),
        jobsScraped: zod_1.z.number(),
        detailPagesVisited: zod_1.z.number(),
        sourcesScraped: zod_1.z.number(),
        blocksDetected: zod_1.z.number(),
    }).optional(),
});
// Track authenticated extension sockets
const authenticatedExtensions = new Set();
...
function calculateFitScore(title, description, budget, rules = DEFAULT_SCORING_RULES, sherlock) {
 ...
 */
function calculateMultiScore(title, description, budget, sherlock) {
    const text = `${title} ${description}`.toLowerCase();
    // ==========================================================================
    ...
    const winLikelihood = Math.round(clientQuality * 0.4 +
        relevance * 0.35 +
        competition * 0.25);
    return {
...
app.prepare().then(() => {
    ...
    const userScrapeStates = new Map();
    function getUserScrapeState(userId) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("DATA_INGEST", async (data) => {
            ...
            try {
                ...
                const fitScore = calculateFitScore(validData.title, validData.description, validData.budget, DEFAULT_SCORING_RULES, sherlockData);
                // Calculate multi-score breakdown (Phase 2)
                const multiScore = calculateMultiScore(validData.title, validData.description, validData.budget, sherlockData);
        ...
        // -------------------------------------------------------------------------
        socket.on("CHECK_JOB_URLS", async (data, callback) => {
            if (!userId) {
        ...
        socket.on("DISCOVERY_COMPLETE", async (data) => {
            ...
            for (const job of jobs) {
                try {
                    ...
                    const multiScore = calculateMultiScore(job.title, job.description, undefined, // No budget from list page
                    sherlockData);
        ...
        socket.on("ENRICHMENT_COMPLETE", async (data) => {
            ...
            for (const job of enrichedJobs) {
                try {
                    ...
                    const multiScore = calculateMultiScore(existing.title, job.fullDescription, // Use full description for scoring
                    undefined, sherlockData);
